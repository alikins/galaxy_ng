...
class TestUiSynclistViewSet(BaseTestCase):
    ...
    def test_synclist_update(self):
        ...
        synclist_detail_url = reverse('galaxy:api:v3:ui:synclists-detail',
                                      kwargs={"pk": synclist1.id})
        self.client.force_authenticate(user=self.user)
        with self.settings(GALAXY_DEPLOYMENT_MODE=DeploymentMode.STANDALONE.value):
            response = self.client.patch(synclist_detail_url, post_data, format='json')
            log.debug('response: %s', response)
            ...
            assert self.user1.username in response.data['users']
            assert self.group1.name in response.data['groups']
    ...
    def test_synclist_detail(self):
        ...
        with self.settings(GALAXY_DEPLOYMENT_MODE=DeploymentMode.STANDALONE.value):
            synclist_detail_url = reverse('galaxy:api:v3:ui:synclists-detail',
                                          kwargs={"pk": synclist1.id})
            response = self.client.get(synclist_detail_url)
            self.assertEqual(response.status_code, http_code.HTTP_200_OK)
            log.debug('response.data: %s', response.data)
    ...
    def test_synclist_delete(self):
        ...
        with self.settings(GALAXY_DEPLOYMENT_MODE=DeploymentMode.STANDALONE.value):
            synclist_detail_url = reverse('galaxy:api:v3:ui:synclists-detail',
                                          kwargs={"pk": synclist1.id})
            log.debug('delete url: %s', synclist_detail_url)

            response = self.client.delete(synclist_detail_url)
