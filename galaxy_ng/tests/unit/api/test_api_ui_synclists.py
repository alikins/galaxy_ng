...
from . import base
from . import rh_auth
from galaxy_ng.tests.unit import rbac_utils
...
class BaseUiSynclistViewSet:
    ...
    def test_synclist_create(self):
        ...
        # rbac_utils.show_obj_perms(self.synclist)

        rbac_utils.show_obj_perms(SyncList)

        rbac_utils.show_user_perms(self.user, self.default_owner_permissions,
                                   klass=SyncList)
        # rbac_utils.show_perms(self.sync
        # self.client.credentials()
        # x_rh_identity = rh_auth.user_x_rh_identity('testuser', '999999')
        # log.debug('x_rh_identity: %s', x_rh_identity)
        # self.client.credentials(X_RH_IDENTITY=x_rh_identity)
