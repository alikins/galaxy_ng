...
def curate_synclist_repository(synclist_pk, **kwargs):
    ...
    collection_versions = \
        CollectionVersion.objects.filter(repositories=synclist.upstream_repository,
                                         collection__namespace__in=namespaces,
                                         is_highest=True) \
        & CollectionVersion.objects.filter(collection__in=synclist.collections.all(),
                                           repositories=synclist.repository)

    if synclist.policy == 'exclude':
        task_kwargs = {'base_version_pk': str(latest_upstream.pulp_id),
