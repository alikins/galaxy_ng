...
requirements:     ## Update python dependencies lock files (i.e. requirements.txt).
	ANSIBLE_SKIP_CONFLICT_CHECK=1 pip-compile --build-isolation $(PIP_COMPILE_UPDATE_SPEC) -o requirements/requirements.common.txt \
		setup.py
	ANSIBLE_SKIP_CONFLICT_CHECK=1 pip-compile --build-isolation $(PIP_COMPILE_UPDATE_SPEC) -o requirements/requirements.standalone.txt \
		setup.py requirements/requirements.standalone.in
	ANSIBLE_SKIP_CONFLICT_CHECK=1 pip-compile --build-isolation $(PIP_COMPILE_UPDATE_SPEC) -o requirements/requirements.insights.txt \
